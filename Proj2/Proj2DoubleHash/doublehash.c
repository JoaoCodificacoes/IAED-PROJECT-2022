...
Res_List *sorted_Insert(Res_List* res,Res_List* head);
...
Res_List* Sort_List(Res_List* head);
...
Res_List* Sort_List(Res_List* head){
    ...
    Res_List* sorted = NULL;
 
    
    Res_List* current = head;
    while (current != NULL) {
 
        Res_List* next = current->next;
 
        current->prev = current->next = NULL;
 
        sorted = sorted_Insert(current,sorted);
...
void Print_Reservations(int i){
    Res_List* temp;
    FlightList[i]->reservation = Sort_List(FlightList[i]->reservation);
...
int hash_func(char* key) {
    ...
    for (h = 0; *key != '\0'; key++, a = a*b % (TABLE_SIZE-1))
        h = (a*h + *key) % TABLE_SIZE;
    return h;
}
